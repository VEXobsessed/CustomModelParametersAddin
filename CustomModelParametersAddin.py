...
def updateParameters():
    # selectionInput = selectionInput.selection(0)
    # paramEntity = paramSelection.entity
    # # paramEntity.name
    # ui.messageBox(paramEntity.name)
    ui.messageBox(globalComp.name)
    # pass
    ...
    def notify(self, args):
        try:
            ...
            if cmdInput.id == 'selection':
                # ui.messageBox(str(selectionInput.selectionCount))
                if selectionInput.selectionCount > 0:
                    # ui.messageBox('It works:\n{}'.format(selectionInput.selection(0).entity.component.name))
                    global globalComp
...
class MyCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            selectionInput = tab1ChildInputs.addSelectionInput('selection', 'Select', 'Component to select')
            ...
            sliderOne = tab1ChildInputs.addIntegerSliderCommandInput('intSlider', 'Integer Slider', 1, 35)         
            sliderOne.valueOne = 7
            ...
            # distanceValueInput.initialValue = 5
...
def stop(context):
    try:
        # # ui.messageBox('end')
        globalComp.modelParameters.itemByName('Model_Param').expression = str(globalSize)
